...
modes as well - they seem like they could be much better encapsulated than they are now.

** Promiscuous state transformations
Since we currently use a single point of truth for all state, having functions that require inputs
from multiple places in the tree means that you have to pass large amounts of unrelated data into
them in the form of the whole state tree. For example, =handle-waypoint-updates= requires only a few
pieces of data but must request access to the entire tree for its processing. This is an antipattern
which could be alleviated by making it easier to request more data and use transforms with
dfifferent arities.

As well, functions that output to the entire tree are probably an anti-pattern. I should take a look
at them and see if they can be split into smaller ones.
